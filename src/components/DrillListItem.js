...
class DrillListItem extends React.Component {
  ...
  render() {
    ...
    if( details ) {
      ...
      const historyButton = <button className="btn-secondary" data-id={details.id} onClick={(e) => this.props.goToHistory(e)}>History</button>
      const setupButton = <button className="btn-secondary" data-id={details.id} onClick={(e) => this.props.goToSetup(e)}>Setup</button>
